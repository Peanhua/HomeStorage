...
def homes_index(page):
 homes = Home.query.order_by(Home.name).paginate(page=page, per_page=get_items_per_page(), error_out=False)
 if homes.page > 1 and homes.page > homes.pages:
...
def myhomes_view(home_id, users_page, products_page):
    ...
    homeuserids = [ u.user_id for u in home.users]
    homeusers   = User.query.filter(User.user_id.in_(homeuserids)).order_by(User.name).paginate(page=users_page, per_page=get_items_per_page(), error_out=False)
